...
export class AuthController {
  ...
  async login(@Res() response, @Body() auth: ILogin) {
    const document = await this.authService.login(auth);
    return response.status(document.status).json(document);
  }
  @Post('/register')
  async register(@Res() response, @Body() auth: IAuth) {
    const document = await this.authService.register(auth);
    return response.status(document.status).json(document);
  }
  @Post('/register/organization')
  async registerUserAndOrganization(
    @Res() response,
    @Body() auth: IAuthOrganization,
  ) {
    const document = await this.authService.registerWithOrganization(auth);
    return response.status(document.status).json(document);
  }
  ...
  async getUserInformation(@Req() request: Request, @Res() response) {
    const document = await this.authService.getUserInformation(request.user);
    return response.status(document.status).json(document);
  }
}

...
export class OrganizationService {
    ...
    async create(organization: Organization): Promise<IResponse> {
        try {
            ...
            let _organization = await document.save();
    ...
    async readAll(): Promise<IResponse> {
        try {
            let documents = await this.organizationModel.find().exec();
    ...
    async readById(id): Promise<IResponse> {
        try {
            let document = await this.organizationModel.findById(id).exec();
    ...
    async delete(id): Promise<IResponse> {
        try {
            let document = await this.organizationModel.findByIdAndRemove(id);
            return { status: HttpStatus.OK, data: document };
        } catch (error) {
            return { status: HttpStatus.BAD_REQUEST, reason: 'We have a problem try again' };
        }
    ...
    async update(id, organization: Organization): Promise<IResponse> {
        try {
            let document = await this.organizationModel.findByIdAndUpdate(id, organization)
            return { status: HttpStatus.OK, data: document };
        } catch (error) {
            return { status: HttpStatus.BAD_REQUEST, reason: 'We have a problem try again' };
        }

...
export class AreasService {
  constructor(
    @InjectModel(Areas.name) private areasModel: Model<AreasDocument>,
  ) {}
  ...
  async readAll(): Promise<IResponse> {
    try {
      const documents = await this.areasModel.find().exec();
  ...
  async readById(id): Promise<IResponse> {
    try {
      const document = await this.areasModel.findById(id).exec();
  ...
  async update(id, area: Areas): Promise<IResponse> {
    try {
      const document = await this.areasModel.findByIdAndUpdate(id, area);
      return { status: HttpStatus.OK, data: document };
    } catch (error) {
      return {
        status: HttpStatus.BAD_REQUEST,
  ...
  async delete(id): Promise<IResponse> {
    try {
      const document = await this.areasModel.findByIdAndRemove(id);
      return { status: HttpStatus.OK, data: document };
    } catch (error) {
      return {
        status: HttpStatus.BAD_REQUEST,
  ...
  async getAreas(countryId: string): Promise<IResponse> {
    try {
      ...
      const document = await this.areasModel.aggregate([match, projection], {});
      const roots = document.filter((v: Areas) => v.group);
      roots.forEach((value) => {
        value.areas = document.filter(
          (area) => String(area.parentGroupId) == String(value._id),
        );
      });

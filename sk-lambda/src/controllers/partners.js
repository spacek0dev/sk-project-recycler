const { create, readAll, readById, remove, update } = require("../services/partners");
...
const readAllDocuments = async (req, res) => {
  try {
    const document = await readAll(req.query.page, req.query.pageSize, req.query.organizationId);
    res.status(document.status).send(document);
  } catch (error) {
    res.status(500).send(error);
  }

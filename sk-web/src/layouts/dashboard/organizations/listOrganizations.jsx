...
const ListOrganizations = ({ data = [], count = 0, onChangePage }) => {
  ...
  const prepareDataToExport = () => {
    ...
    data.map((value, index) => {
      ...
      _[translate("ubication")] = `${value.address} - ${value.areasId.name}`;
  ...
  const sortByColumn = (value) => {
    ...
    switch (value) {
      case "name":
        ...
        sortList(appTypes.organizations, _name);
        break;
      case "ubication":
            ...
            {data.map((value, index) => {
              return (
                <tr className={styles.tableBodyItem} style={{ color: items.find((v) => v == index) != undefined ? "#2563eb" : "#525f7f" }} key={index}>
                  ...
                  >
                    {value.address} {value?.areasId.name}
                  </td>
                </tr>
              );
            })}

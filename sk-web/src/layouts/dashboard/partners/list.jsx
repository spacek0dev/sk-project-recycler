...
const ListPartners = ({ data = [], count = 0, onChangePage }) => {
  ...
  const initHeader = () => {
    const _headers = [
      ...
      { name: translate("address"), key: "address", sort: null },
  ...
  const prepareDataToExport = () => {
    ...
    data.map((value, index) => {
      ...
      _[translate("email")] = value.email;
      _[translate("phone")] = value.phone;
      _[translate("address")] = `${value.address}`;
      _[translate("logo")] = `${value.logo}`;
  ...
  const sortByColumn = (value) => {
    ...
    switch (value) {
      case "name":
        ...
        sortList(appTypes.partners, _name);
        break;
      case "address":
        let _address = data.sort((a, b) => {
          return item.sort ? a.address.localeCompare(b.address) : b.address.localeCompare(a.address);
        });
        sortList(appTypes.partners, _address);
        break;
      case "email":
        let _email = data.sort((a, b) => {
          return item.sort ? a.email.localeCompare(b.email) : b.email.localeCompare(a.email);
        });
        sortList(appTypes.partners, _email);
        break;
      case "phone":
        let _phone = data.sort((a, b) => {
          return item.sort ? a.phone.localeCompare(b.phone) : b.phone.localeCompare(a.phone);
        });
        sortList(appTypes.partners, _phone);
        break;
